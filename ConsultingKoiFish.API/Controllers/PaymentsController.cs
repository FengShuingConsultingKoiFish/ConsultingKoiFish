...
namespace ConsultingKoiFish.API.Controllers
{
    ...
    public class PaymentsController : BaseAPIController
    {
        ...
        public PaymentsController(IVnPayService vnPayService,
            ...
            IConfiguration configuration, IAdvertisementPackageService advertisementPackageService,
        ...
        public async Task<IActionResult> PurchasePackage(AdvertismentPackagePurchaseDTO packageViewDTO)
        {
            ...
            var package =
                await _advertisementPackageService.GetPackageById(packageViewDTO.PackageId,
                    OrderImage.DatetimeDescending);
            if (package == null)
            {
            ...
            var responseUrl =
                await httpClient.PostAsJsonAsync(_configuration["VnPayConfiguration:RequestPaymentUrl"], vnpRequest);
        ...
        public async Task<IActionResult> ResponsePayment()
        {
            ...
            try
            {
                ...
                var package =
                    await _advertisementPackageService.GetPackageById(packageId, OrderImage.DatetimeDescending);

                if (action.Equals("gia han"))
                {
                    var extendedPurchasedPackage =
                        await _purchasedPackageService.GetUnavailablePackageForMember(packageId, userId);
                    var extendPackage =
                        await _purchasedPackageService.ExtendPurchasedPackage(extendedPurchasedPackage, userId);
...
<p>- Chân thành xin lỗi vì sự bất tiện này. Và xin cảm vì đã đồng hành cùng chúng tôi.</p>"
                        ...
                        var response =
                            $"Hệ thống đã gửi mail đến Email: {user.Email}. Xin vui lòng kiểm tra Email của bạn,";
...
<p>- Chân thành xin lỗi vì sự bất tiện này. Và xin cảm vì đã đồng hành cùng chúng tôi.</p>"
                    ...
                    var createdPurchasedPackage =
                        await _purchasedPackageService.CreatePurchasedPacakge(createdPurchasedPackageDTO);
...
<p>- Cảm ơn đã sử dụng dịch vụ của chúng tôi.</p>"
            ...
            catch (Exception ex)
            {
                ...
                Console.ResetColor();
                return Redirect(
                    $@"{responseUrlFe}?responseMessage=Da xay ra loi trong qua trinh xu ly. Vui long thu lai sau.");
            }
        }
